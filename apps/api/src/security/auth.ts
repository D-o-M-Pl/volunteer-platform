...
export type AppRole = 'ADMIN' | 'ORGANIZATION' | 'VOLUNTEER';
...
function config() {
  ...
  if (!issuer || !audience || !jwksUri) {
    throw new Error('AUTH_ISSUER, AUTH_AUDIENCE and AUTH_JWKS_URI are required');
  }
  verifier ??= createRemoteJWKSet(new URL(jwksUri), {
...
function normalizeRoles(value: unknown): AppRole[] {
  ...
  return raw.filter((role): role is AppRole =>
    role === 'ADMIN' || role === 'ORGANIZATION' || role === 'VOLUNTEER');
}
...
export function hasRole(request: FastifyRequest, ...roles: AppRole[]) {
  return request.auth.roles.some((role) => roles.includes(role));
}
